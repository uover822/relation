...
describe('relation', function () {
  ...
  it('query', {timeout: 8888}, function (done) {
    ...
    // Load the info plugin
      ...
      .act('role:relation,cmd:query,name:seneca', function (ignore, out) {
      ...
      .act('role:relation,cmd:get,name:seneca', function (ignore, out) {
        expect(out.id).to.equal('seneca')
        expect(out.giturl).to.contain('senecajs/seneca.git')
      ...
      .act('role:relation,cmd:get,name:seneca,update:true', function (ignore, out) {
        expect(out.id).to.equal('seneca')
        expect(out.giturl).to.contain('senecajs/seneca.git')
